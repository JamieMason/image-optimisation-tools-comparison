...
function exists(toolName, imageName) {
  return stat(getPath(toolName, imageName)).then(() => true, () => false);
}
...
function getType(imageName) {
  return imageName.endsWith('png')
    ? 'PNG'
    : imageName.endsWith('gif') ? 'GIF' : 'JPG';
}
